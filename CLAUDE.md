...
node --test tests/*.test.js          # Run all 329 test files — suite ONLY; does NOT
...
recorded in `.ctoc/coverage-baseline.json`, which is **97** today (real src line
coverage measured 97.98%, SCOPED to `src/**`). The gate scopes coverage with
...
`minPct`, **97** today (measured 97.98% src line coverage, scoped to `src/**`),
