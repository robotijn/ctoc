...
node --test tests/*.test.js          # Run all 284 test files — suite ONLY; does NOT
...
recorded in `.ctoc/coverage-baseline.json`, which is **88** today (real src line
coverage measured 91.15%, SCOPED to `src/**`). The gate scopes coverage with
...
ctoc/
  ...
  src/                   Source code directory
    ...
    lib/                 92 JS modules (state, quality, security, planning, UI, analysis)
...
`minPct`, **88** today (measured 91.15% src line coverage, scoped to `src/**`),
