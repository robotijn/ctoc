...
node --test tests/*.test.js          # Run all 299 test files — suite ONLY; does NOT
...
recorded in `.ctoc/coverage-baseline.json`, which is **95** today (real src line
coverage measured 95.47%, SCOPED to `src/**`). The gate scopes coverage with
...
`minPct`, **95** today (measured 95.47% src line coverage, scoped to `src/**`),
