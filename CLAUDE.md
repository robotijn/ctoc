...
node --test tests/*.test.js          # Run all 304 test files — suite ONLY; does NOT
...
recorded in `.ctoc/coverage-baseline.json`, which is **96** today (real src line
coverage measured 96.32%, SCOPED to `src/**`). The gate scopes coverage with
...
`minPct`, **96** today (measured 96.32% src line coverage, scoped to `src/**`),
