...
- **`ALTER TYPE` on an existing column to a wider type**: since Postgres 9.2 widening is metadata-only IF the binary representation is compatible (e.g., `varchar(50) → varchar(100)`, or dropping a `varchar`/`numeric` length limit; `int → bigint` is NOT — it changes the on-disk width and rewrites). Reviewer must check the compatibility matrix.
