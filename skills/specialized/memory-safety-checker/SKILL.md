...
- **Memory-safe by default**: United States cybersecurity agencies (CISA and the NSA) recommend that new systems code be written in a memory-safe language — Rust, Go, C#, Java, Swift, Python. The empirical anchor for that push is the ~70% figure, reported independently by two vendors: Microsoft reported (2019) that ~70% of the security vulnerabilities it assigns a CVE each year are memory-safety issues, and the Chromium project reports that ~70% of its serious/high-severity security bugs are memory-safety problems (Google, 2020).
