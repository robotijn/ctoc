...
- **Signed build provenance — and know which SLSA Build Level you actually reach.** `actions/attest-build-provenance@<sha>` produces an in-toto provenance predicate, signed by a short-lived Sigstore certificate, that binds the artifact digest to the workflow run; on GitHub-hosted runners this reaches **SLSA Build Level 2** (dedicated build infrastructure plus signed provenance). **Build Level 3 additionally requires that the provenance-signing key be inaccessible to the user-defined build steps** — which the standard attestation, signing inside the same tenant-controlled job, does NOT provide. For L3 use `slsa-framework/slsa-github-generator`'s isolated reusable workflow, which keeps signing out of the build steps. Verify on consumption with `gh attestation verify` or `slsa-verifier`. As of its v4, `attest-build-provenance` is a thin wrapper over the general `actions/attest`; existing workflows may keep using it, but new implementations should prefer `actions/attest` directly.
...
- **Block `pull_request_target` and `workflow_run` foot-guns.** These triggers run with the **base-repo** secrets and write token. `actions/checkout` with no `ref` safely checks out the **base branch** by default (the trigger runs in the base-branch context); the confused-deputy vulnerability appears only when the workflow **explicitly** checks out the PR's code (`ref: ${{ github.event.pull_request.head.sha }}`) and then runs it with that elevated token. If you must, check out an explicit, vetted ref and gate by author / fork.
