...
- **Pod Security Standards `restricted` is the baseline**, not `baseline`. PSA admission has been stable since Kubernetes 1.25 and is on by default in most managed offerings (EKS / GKE / AKS) in 2026. PodSecurityPolicy is fully gone — flag any manifest referencing the deprecated API. Restricted profile mandates: `runAsNonRoot: true`, a non-zero `runAsUser`, `allowPrivilegeEscalation: false`, all Linux capabilities dropped (`capabilities.drop: ["ALL"]`, only `NET_BIND_SERVICE` re-addable), seccomp `RuntimeDefault` (or `Localhost`), a restricted volume-type allowlist, and — inherited from `baseline` — no `hostPath` / `hostNetwork` / `hostPID` / `hostIPC`. Note: `readOnlyRootFilesystem` is NOT one of the PSS restricted controls — the official Pod Security Standards do not include it in any profile. This skill enforces `readOnlyRootFilesystem: true` anyway as its own Category 11 control, one notch stricter than PSS.
