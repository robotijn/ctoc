...
16. **This slice does not edit CLAUDE.md.** At authoring, adding a `src/lib` module and two
    test files meant hand-editing the counts documented in CLAUDE.md so `doc-counts.test.js`
    would pass. Plan 00215 shipped between authoring and this rebase: those counts are now
    GROWING tallies GENERATED by `release.js` (`computeDocCounts`), and `doc-counts.test.js`
    cross-checks the generator against an independent disk walk without parsing the CLAUDE.md
    literal — so adding files can no longer break the gate, and hand-editing the number is
    the exact tax 00215 removed. CLAUDE.md is therefore dropped from `files:` and from every
    step. The change is a route correction only; the plan's intent and acceptance criteria
    are unchanged.
