...
### Security Review
- [ ] **No injection surface added:** `splitCliArgs` only splits/returns strings; it
      never evaluates or shells out.
- [ ] **Prototype-pollution:** returns a plain array of strings; no object merge, no
      `__proto__` handling; downstream `buildAddSpec`/`parseTaskArgs` already populate
      named fields only (`menu-screens.js:1427`).
- [ ] **Control-char safety:** unchanged — the render layer's existing `stripCtl`
      guards still apply to any stored value at display time.
- [ ] **Fail-safe:** non-array input → `[]` (route falls through to the dashboard), never
      a throw.
