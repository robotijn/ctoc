...
- [ ] **Scenario: Every non-`_shared` agent file begins with frontmatter at byte 0**
...
- [ ] **Scenario: `cto-chief` loads with its declared read-only tool set (live)**
...
- [ ] **Scenario: Each of the 5 scouts loads with `model: haiku` (live)**
  Given the 5 corrected scout agent files
  When the plugin registers agents at session start
  Then each scout's live registration reports `model: haiku`, not the session model

- [ ] **Scenario: A heading containing YAML-special characters does not corrupt the move**
...
- [ ] **Scenario: The anchored parser goes red on the current (pre-fix) tree**
...
- [ ] **Scenario: A synthetic heading-first fixture is rejected, not partially parsed**
...
- [ ] **Scenario: The anchored parser goes green after Story A's fix**
...
- [ ] **Scenario: No `_shared` fragment is dispatchable after the fix**
...
- [ ] **Scenario: The pre-fix discovery walker currently DOES register `_shared` fragments (red baseline)**
...
- [ ] **Scenario: The exclusion generalizes to a new file, not just the 4 known fragments**
