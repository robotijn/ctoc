...
### Step 8: TEST
- [ ] Write `tests/ctoc-audit-w05-gate3-review-to-done.test.js` FIRST (TDD RED):
      the 7 behavior cases above, using real temp plan fixtures + real artifact
      JSON (no doubles).
- [ ] Confirm RED: today `validateReviewToDone` returns `valid:true` for all
      fixtures, so cases 1-3, 5, 6 fail; case 7's bad sibling is currently
      approved.

### Step 9: PREPARE
- [ ] Confirm `s1` is built (`readVerifyEvidence` exported from
      `./step-13-verify`); if not, this slice's `depends_on` is unmet — do not
      start.
- [ ] Confirm `validateStepsComplete` is callable in-module and returns the
      per-step `checklist` shape used above.

### Step 10: IMPLEMENT
- [ ] `src/lib/plan-validator.js`: add `require` of `readVerifyEvidence` from
      `./step-13-verify`.
- [ ] `src/lib/plan-validator.js`: rewrite `validateReviewToDone` body — marker
...
### Step 11: REVIEW
- [ ] Self-review: verify `valid` can be both `true` (M2) and `false` (M1) — no
      always-reject, no always-pass; error strings are specific; no change to
      `approveSubplans`.

### Step 12: OPTIMIZE
- [ ] Read the plan content once; call `validateStepsComplete` once; single
      `readVerifyEvidence` call; no redundant file reads.

### Step 13: SECURE
- [ ] Walk the Security Review checklist; confirm fail-closed on every
      unreadable/corrupt/stale path; confirm no user-interpolated `RegExp`.

### Step 14: VERIFY
- [ ] Run `node --test tests/ctoc-audit-w05-gate3-review-to-done.test.js` → green.
- [ ] Run full suite `node --test tests/*.test.js` → `# fail 0`, `0 skipped`.
- [ ] Coverage ≥80% on `validateReviewToDone`.

### Step 15: DOCUMENT
- [ ] Update the JSDoc on `validateReviewToDone` to describe the three rejection
      conditions and the pass condition.

### Step 16: FINAL-REVIEW
- [ ] Confirm the review→done gate can now fail on marker/box/evidence, passes a
