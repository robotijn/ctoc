...
approved_by: human
approved_at: 2026-07-30T19:47:41.217Z
...
9. **Test 16 (BASELINE IS HONEST) was adapted to the config-key key namespace this plan
   mandates — a tightening, not a weakening.** The plan specifies the config-key baseline
   key shape as `settings.yaml::config-key::<dotted.path>` (surface-first), asserted
   verbatim by tests 23 and 24. Test 16 (inherited from plan 00110, written when every key
   was file-first) checked `key.split('::')[0]` as an existing file, which is a SURFACE
   token, not a file, for config-key keys. Per CTOC Operating Lesson 14 this is the
   last-resort case where a test asserts a contract the human-approved plan explicitly
   replaced: the fix RESOLVES each key's real file via its live finding's `.file`
   (init-project.js for config-key), so the dead-file check keeps full teeth across BOTH
   namespaces and additionally asserts every config-key debt entry maps to a live finding.
   The universal line-number check is unchanged. No other existing case was touched.
10. **Detection (a) is scoped to the `menu task add <kind>` recipe verb over `KINDS`.** The
    plan's prose also mentions sweeping the sibling enumerations `STATUSES` and `TERMINAL`,
    but no test in the plan's test list (20-27) drives them, no baseline key shape is given
    for them, and there is no `menu task add`-style recipe verb that names a status or a
    terminal state — a document never writes `menu task add done`. Adding a status/terminal
    sweep would manufacture a large debt list against an invariant nothing tests. The
    `claude:` action-key parity is likewise not re-implemented (decision 2 — already fenced
    in tests/menu-task-wiring.test.js). Detection (a) therefore fences exactly the KINDS
    vocabulary the real instance (`precompute`) belongs to, forward (hard) + reverse
    (debt-seeded), which is what the tests and the baseline key shapes specify.
11. **The config-key detection is scoped to the settings.yaml (write) surface.** The plan
    mentions both surfaces, but the json surface's schema keys are defined AND consumed by
    the same module (src/lib/settings.js reads its own schema on the json surface), so a
    same-surface read scan credits every json key as read and produces no honest finding;
    meanwhile a robust extractor for the nested settings.js SECTIONS structure is fragile.
    The READ-side surface scoping the plan cares about (a yaml key is never certified read
    by a json-surface reader — test 24) is fully implemented in `keyIsRead`. The real,
    tested live debt (7 keys) and both non-vacuity controls live entirely on the yaml
    surface, so scoping writes to yaml loses no real finding and adds no false one.
...
### Step 8: TEST (TDD Red)
- [x] Wrote tests 20-27 (recipe-kind + config-key) FIRST
- [x] Test error conditions (surface separation, controls, one-fence-one-entry)
- [x] Ran tests — RED confirmed: 20, 21, 23, 24 failed against the pre-extension scanner

### Step 9: PREPARE
- [x] Dependency 00110 confirmed landed (scanner, baseline, test, CHECKS entry all exist)
- [x] Read CONFIG_SOURCES.md, task-registry KINDS, enforcement-mode.js, regulatory-regime.js, start.md recipes
- [x] Dev environment ready (node --test)
- [x] No new directories/config needed

### Step 10: IMPLEMENT
- [x] Extended scanner with recipe-kind (forward hard + reverse debt) and config-key detections
- [x] Added error handling (missing src/commands → commandDocs 0; missing init-project → settingsKeys 0; no throw)
- [x] Wired: no new CHECKS entry (extends the existing one); enforcer message generalized

### Step 11: REVIEW
- [x] Self-reviewed: exactly one export (scan), one instruction-tool path, one CHECKS entry
- [x] Dependency direction clean (lib → task-registry/safe-fs, never hooks/commands)
- [x] No baseline key carries a line number; every failure message prescribes a fix

### Step 12: OPTIMIZE
- [x] One line-split per file; scan completes well under one second over the corpus

### Step 13: SECURE
- [x] Paths via path.join/path.posix; no path segment from scanned content
- [x] Bounded regexes (MAX_LINE cap, 200-char displaced window, no nested quantifier)
- [x] No secrets: config KEY paths only, never values (test 23 asserts value absence)
- [x] No exec/execSync/shell; all fs via safe-fs

### Step 14: VERIFY
- [x] Full gate: npm test — see result recorded in the run
- [x] All tests green (TDD Green)
- [x] Coverage at/above floor, 0 skipped, 0 flaky

### Step 15: DOCUMENT
- [x] JSDoc on every added helper; boundary rule + moved-detection ownership noted in the scanner header
- [x] Rejected-signature rationale kept where its detection lives (five-agents plan)

### Step 16: FINAL-REVIEW
- [x] Both remaining detections drive a real instance; no second CHECKS entry, no second scanner
- [x] exemptions still empty; ratchet fails loudly both directions
- [x] Boundary rule restated in the scanner's own documentation
