...
15. **Source-label uses `settings.ENVIRONMENT_PROFILES[env]`, not
    `settings.getEnvironmentProfile(env)`.** Both answer the identical question —
    "does the chosen profile name `workflow.enforcementMode`?" — with identical
    honesty. The plan sketch named `getEnvironmentProfile`, but wiring it would have
    paid down a dead-export (it is currently baselined as unreachable), dropping the
    export-reachability count 65 → 64 and REQUIRING an edit to
    `.ctoc/export-reachability-baseline.json`, which is NOT in this plan's `files:`.
    `ENVIRONMENT_PROFILES` is already exported AND already reachable, so reading it
    directly keeps the change strictly within the declared file set and touches no
    ratchet artifact. `getEnvironment` already validated `env`, so the direct index
    is safe (guarded with `|| {}`).
16. **`enforcement-mode.js` exports only `resolveEnforcementMode`, `ENFORCEMENT_MODES`
    and `DEFAULT_ENFORCEMENT_MODE`.** `resolveEnforcementMode` has its live caller in
    `PreToolUse.Edit.js`; the two constants are consumed internally by the resolver,
    so the export-reachability fence counts them reachable (the dead-export count did
    not rise). The flat parser and the tier helpers stay module-private, tested only
    through `resolveEnforcementMode` against real temp-dir fixtures.
...
### Step 9: PREPARE
- [x] Install dependencies if needed (none — leaf module on path/safe-fs/settings)
- [x] Check prerequisites (settings.js exports getSetting/readRawSettings/getEnvironment/ENVIRONMENT_PROFILES confirmed; hooks.json registers PreToolUse.Edit.js)
- [x] Verify dev environment ready
- [x] Create directories/config if needed (none)

### Step 10: IMPLEMENT
- [x] Implement the feature according to requirements (enforcement-mode.js resolver + flat parser; Edit.js fail-soft require, buildSoftWarnMessage, mode/mode_source in every allow/block, single resolution threaded through, three-way step-5 tail preserving the denial explanation on strict; Task.js comment; enforcement-log.js header)
- [x] Add error handling (resolver never throws; every tier fails closed to strict)
- [x] Wire up integration points (Edit.js enforce() calls resolveEnforcementMode — the live call site, shipped in this slice)

### Step 11: REVIEW
- [x] Self-review all new code (lib never imports a hook; exactly one production caller; mode read at one decision point; ledger/verify/streaming guards return before mode matters; strict branch still computes denial)
- [x] Verify integration points work together
- [x] Check error handling completeness

### Step 12: OPTIMIZE
- [x] Remove redundant operations (one resolution per invocation; parser exits early on the matched key)
- [x] Optimize critical paths
- [x] Simplify complex code

### Step 13: SECURE
- [x] Validate inputs (mode checked against frozen allowlist; non-string root → strict; no user-controlled path segment)
- [x] Sanitize outputs (resolver emits no message; no path/stack leaks)
- [x] No secrets in code (none read/logged/written)
- [x] Safe file operations (read-only via safe-fs; linear per-line regex, no ReDoS)

### Step 14: VERIFY
- [x] Run lint + type check (tsc --checkJs clean; typecheck baseline held at 0)
- [x] Run ALL tests (TDD Green) — npm test exit 0, failed 0
- [x] Check coverage >= 80% (99.08% scoped src/**, threshold 99%)
- [x] 0 skipped, 0 flaky tests

### Step 15: DOCUMENT
- [x] Update relevant documentation (CLAUDE.md per-project tuning + step-5 + floor; template Enforcement Mode subsection; CONFIG_SOURCES.md reader named)
- [x] Add JSDoc comments to new functions (every export of enforcement-mode.js; buildSoftWarnMessage)
- [x] Update CHANGELOG if needed (n/a — no CHANGELOG stage for this slice)

### Step 16: FINAL-REVIEW
- [x] Verify steps 8-15 completed correctly
- [x] All quality checks passed
- [x] Manual verification if needed
- [ ] Ready for human review (Gate 3 — the human's decision)
