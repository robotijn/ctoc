...
approved_by: human
approved_at: 2026-07-31T00:42:52.500Z
...
## STEP 14 — EXPORT fence RESOLVED (2026-07-31); only a PRE-EXISTING, external block remains

The EXPORT-fence movement the earlier checkpoint flagged is now RESOLVED (the human expanded
the declared set; the three exports are wired — see the Decisions section below). Measured
state after resolution: FILE fence **17**, EXPORT fence **exactly 65** (baseline UNCHANGED,
nothing grown), both fences green, and every compliance / runner / helper / behavioral test
passes. Coverage 99.07% (≥ 99 floor), 0 skipped.

The full `npm test` still exits non-zero, but for ONE cause that is NOT this plan's and NOT
caused by any edit here: the `iron-loop-enforcer` live-repo self-check flags
`gate-destinations-approved` — **`plans/todo/00168-a-claim-outlives-the-executor-that-made-it.md`
changed in the specification after approval** (a hash-mismatch against its approval ledger).
Proven independent of this plan: with ALL of 00191's changes stashed, the identical block
fires at clean `HEAD` (commit e533635, "00166/00168 held as dead-foundation"). Plan 00168 is
outside this plan's declared files, and its fix is a human re-approval (revert its spec to the
approved text, or re-approve it) — a gate this executor must not cross. **This is a fork for
the human:** resolve 00168 (or authorize adding it to scope) to green the whole-repo gate,
after which 00191 completes cleanly; nothing further in 00191 is red.

## Decisions Taken Under Ambiguity — the three EXPORT-fence exports (2026-07-31, scope expanded by the human)

The human added `.ctoc/export-reachability-baseline.json`, `src/lib/eu-ai-act-helpers.js`,
`src/lib/gdpr-helpers.js`, `src/lib/eu-ai-act-agent-runner.js`, `src/lib/gdpr-agent-runner.js`
to this plan's declared files. Wiring the seven-file closure reachable had surfaced three
previously-out-of-scope, uncalled helper exports. Per-export judgment against what each
runner honestly needs (the runners are the executable enforcement of the compliance agents'
prose contracts, and each helper is that contract's deterministic core):

1. **`gdpr-helpers.js#mapPiiFieldToArticles` → WIRED into `gdpr-agent-runner.js`.** First
   attempt (derive a MISSING `gdpr_article` from a `pii_field`) was caught FAKE by test case
   11 and corrected: the seam (`compliance-integration.js`, outside the declared set)
   pre-validates GDPR findings via `partitionValidGdpr` and DROPS any without a valid
   `gdpr_article` BEFORE the runner runs, so a derive-when-missing call in the runner can never
   fire through the live seam path — exactly the no-op caller the directive forbids. The honest
   wiring that DOES fire: a finding flagged under one Article (e.g. `GDPR-17`) that also names a
   `pii_field` survives the seam and the runner ENRICHES it with the COMPLETE triggered-Article
   set (`GDPR-6, GDPR-13, GDPR-17` for `email`) in the Inbox context — the mapping's actual
   purpose (`agents/compliance/gdpr-agent.md`), surfaced to morning review. DELETE was rejected:
   `mapPiiFieldToArticles` is legitimate, tested logic used by three test files
   (`gdpr-helpers.test.js`, `compliance-fixtures.test.js`, `gdpr-agent-definition.test.js`) and
   named in the agent contract. Proven firing end-to-end through the shipped recipe by case 11.
2. **`eu-ai-act-helpers.js#classifyFromPlanText` → WIRED into `eu-ai-act-agent-runner.js`.** The
   runner routes findings that must carry `risk_class` (the Inbox context emits it). A finding
   derived from plan prose naturally lacks a pre-computed class. The runner now classifies an
   unclassified finding from its own `message`/`plan_text` via the deterministic classifier.
   Executable half of `agents/compliance/eu-ai-act-agent.md`'s documented use. Proven by case 12.
3. **`eu-ai-act-helpers.js#readEnforcementDates` → WIRED into `eu-ai-act-agent-runner.js`.** The
   runner reads the active regime profile's enforcement milestones (fail-open) once per run and
   surfaces the finding's applicable date in the human-facing Inbox context — the module's stated
   purpose ("dates READ FROM THE PROFILE, never hardcoded"). Proven by case 13.

None was DELETED: each is the deterministic core of a real derivation the executable runner
should perform, and wiring each is honest use grounded in the agents' own contracts. Result,
MEASURED: the EXPORT-fence dead count returns to exactly **65** (baseline UNCHANGED — nothing
grown), the FILE fence stays at **17**, and the three exports are LIVE. No file outside the
expanded declared set was touched.
