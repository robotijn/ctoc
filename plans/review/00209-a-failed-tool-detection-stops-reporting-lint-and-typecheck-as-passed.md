...
9. **Justified test change in `tests/quality-agent-coverage.test.js` (added to the declared
   files by the human).** The case *"returns action:push when every Tier 1 check passes on a
   clean project"* (`:699`) called `runTieredChecks({})` with an EMPTY tools map and asserted
   `allPassed:true` / `action:'push'`. That assertion encoded the exact false-green this plan
   removes: a zero-tool DETECTION producing a passing, push-authorizing tier. The contract it
   asserted was replaced at Gate 2 — an empty tools map is now NOT VERIFIED, not a pass. The
   test is TIGHTENED toward the real "clean project" behaviour rather than loosened: the case
   now passes a real tools map with lint + typecheck + test present and PASSING, and additionally
   asserts `res.tier1.lint.ran === 1` / `typecheck.ran === 1` — so it still proves the push path,
   but for a project whose checks actually RAN, not a vacuous empty one. The assertion was not
   inverted, deleted, or weakened. This is the only case in that file that relied on the
   empty-map-is-pass shortcut; the sibling block case (`:717`, `{ js: { lint: failCmd() } }`)
   still asserts `action:'block'` and remains correct (a failing lint, `ran:1`).


## Execution Record (Steps 8-16)

- **Step 8 TEST (RED captured):** `tests/vacuous-verification.test.js` written first; 11 of
  12 cases RED for the right reason. Load-bearing evidence — case 9, the composition/persist
  verdict on an empty tools map: `AssertionError: 'pass' !== 'fail'` (an empty detection
  currently yields a PASSING tier). Case 1: `runLint({})` returned `passed:true` (defect).
  Case 12 (runSecurityScan scope guard) was green from the start, as intended.
- **Step 9 PREPARE:** every cited line re-verified against disk before editing — runLint
  586-607, runTypecheck 612-632, updateTierStatus 1482-1485, setCompleted defaults 1671-1672
  all matched the plan. Runner is `runConfiguredCommand` (confirmed). The undetermined idiom
  (`undeterminedTestsResult`) shape adopted. `countSubstantiveChecks` in step-13-verify.js is
  shaped for the VERIFY result object and is NOT importable here — the PATTERN (a ran-count
  carried with the verdict) was adopted, per Decision 5.
- **Step 10 IMPLEMENT:** `runLint`/`runTypecheck` now carry a `ran` count; zero tools →
  `notVerifiedLint`/`notVerifiedTypecheck` (`passed:false, undetermined:true, ran:0,
  errors:null`). The two setCompleted `|| { passed:true }` defaults inverted to the
  failure-shaped factories. `updateTierStatus` needed no change — it persists `checks: tier1`,
  which now carries the ran-counts inside each check object. Two factories exported (called by
  live internal code, so both reachability fences pass).
- **Step 11 REVIEW:** no path returns `passed:true` without `ran >= 1`; `errors` is `null`
  wherever nothing was measured, never `0`. New tests green in isolation (12/12).
- **Step 13 SECURE:** the not-verified messages echo NO command output — they describe the
  ABSENCE of a tool. No new code path bypasses `runConfiguredCommand`'s shell-operator refusal.
- **Step 14 VERIFY:** `npm test` → EXIT 0, coverage 99.09% (threshold 99%, PASS), skipped 0,
  failed 0, 0 flaky. (First run had failed 1 — the scope-blocker test that encoded the defect;
  fixed the honest way per Decision 9 after the human added the file to scope.)
- **Step 15 DOCUMENT:** CLAUDE.md quality-gate section records the NOT VERIFIED / fail-the-tier
  rule; test-file count updated 499 → 500 in both places.

## Findings (reported, not fixed)

1. **`runSecurityScan` (`:1059`) does NOT share the identical defect, but a milder sibling
   exists.** It always runs the pure-JS `SecretsScanner` (no external tool needed) and folds
   every skip into `skipped[]`, surfaced in the summary — so it is not "zero checks ran →
   silent pass" the way lint/typecheck were. However `passed = critical === 0 && high === 0`
   (`:1449`) means a run whose secrets scanner examined ZERO files (an empty push delta, no
   `allFiles`) still returns `passed:true`. That is a weaker instance of the same class worth a
   separate look. Left for the human to schedule, per Decision 6.

## SCOPE BLOCKER — RESOLVED

The human added `tests/quality-agent-coverage.test.js` to this plan's declared files, so the
one justified test fix was made (see Decision 9): the empty-tools case at `:699` now passes a
real clean-project tools map (lint + typecheck + test present and passing) and additionally
asserts `ran === 1` for lint and typecheck. It was the only case relying on the empty-map
shortcut. Full gate green after the fix (see Execution Record, Step 14, updated below).
