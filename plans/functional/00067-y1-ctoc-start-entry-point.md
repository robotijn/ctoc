...
Notes appended after approval invalidated the ledger hash; returning for the human re-approval he authorised.

---

---
revision: 1
rejection_reason: "Notes appended after approval invalidated the ledger hash; returning for the human re-approval he au"
tag: rejected
approved_by: human
...
### Landed early — the decision matrix in the streaming screen text (2026-07-18)

**One piece of this plan shipped ahead of the rest, as a focused defect repair.**
The human opened the menu, could not read the question, and reported the product as
broken. He was right: `precomputedOptionDescription` in `src/lib/streaming-gate.js`
flattened every option's separate pros, cons and recommendation fields into one
run-on sentence, so the critique fleet's reasoning was present in the stored data
and invisible on screen. A person blocked on an unreadable screen cannot wait for
the entry-point rename, so the matrix rendering was built and verified on its own.

What landed: `precomputedQuestionMatrix` and its helpers render each question's
options as the canonical box-drawing decision matrix from
`.ctoc/ask-me-questions.md` — columns Option, Pros, Cons, Recommendation — into the
screen text, matrix first and the question sentence after it. Both surfaces that
share `precomputedQuestionParts` get it: the streaming gate screen and the
open-a-plan screen. Total rendered width is capped by the named constant
`MATRIX_TOTAL_WIDTH` (88 characters including every border), long text wraps inside
its cell, and nothing is dropped to fit. Every cell value is subagent-authored, so
it keeps passing `stripCtl` and additionally has newlines and box-drawing
characters neutralised, which stops a hostile field from forging a matrix row.

What did NOT land: everything else in this plan — the entry-point rename, the
session-context and README changes, and the rest of the open-prompt work. The
flattened one-sentence description is unchanged and still correct for the option
descriptions handed to the question interface; only the screen text gained the
matrix.

### Decision 2 — the Recommendation cell derives a short reason, never copies the Pros

The stored question schema has no dedicated recommendation-reason field; it has
`pros`, `cons`, `description` and a boolean `recommended`. A first attempt restated
the recommended option's pros in this cell. Rendered against the real questions in
`.ctoc/streaming/questions/`, that made the cell a second, narrower copy of the cell
beside it, cut off mid-word. The cell now carries a DERIVED clause: when the
critique recorded a confidence level — its own convention for how well grounded a
finding is — the clause is "Recommended — confidence high, on the reasoning in the
Pros column."; otherwise it names the option as the highest-quality one and points
at the Pros column. It states the verdict and where the argument lives, and never
repeats the argument. Exactly one Recommendation cell is filled; the rest are empty.

### Decision 3 — the Option cell is the label alone; the description leaves the matrix

An option's `description` has no column of its own in the four-column contract. It
was first rendered in the Option cell beneath the label. Against real data that was
a disaster: the descriptions written by the critique fleet are paragraphs of file
and line citations, and a narrow Option column wrapped one of them twenty lines down
the page, making the matrix taller than the screen and unreadable.

The Option cell now carries the LABEL ALONE, and the Option column is the narrowest
of the four. The description is not lost — it still rides in the flattened
one-sentence description handed to the question interface, which is where the human
reads the evidence while choosing an option. The matrix is the scanning surface and
stays scannable; the evidence lives one layer down where there is room for it. On
the real questions in this repository this cut the rendered matrix from sixty-one
lines to thirty-three.

### Decision 4 — a token wider than its column breaks at a separator, never mid-word

Wrapping is on word boundaries. A single token wider than its column still has to
break somewhere; it breaks after the last separator inside the column window — a
slash, backslash, hyphen, underscore, period, colon, comma or semicolon — so a file
path reads as `src/lib/task-` then `reconcile.js:637-649` rather than
`src/lib/task-reconci` then `le.js:637-`. A fragment shorter than four characters is
not worth a line, so a separator that close to the start of a token is skipped and a
later break point is used. A token with no separator at all breaks at the column
width, which is the only remaining option. Nothing is ever silently truncated: every
cell renders its whole text, and a test asserts the rendered cells reconstruct the
source exactly, on the real critique-fleet data.
