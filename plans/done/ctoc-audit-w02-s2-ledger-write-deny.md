---
approved_by: human
approved_at: 2026-07-13T20:53:24.175Z
...
### Step 8: TEST (TDD Red)
- [x] Write `tests/ctoc-audit-w02-s2-ledger-write-deny.test.js`. The predicate cases
...
### Step 9: PREPARE
- [x] Locate the sandbox-project helper pattern in `e2e-enforcement-and-gates.test.js`
      (writes a minimal `.ctoc/` so `isCtocProject` returns true) and reuse it.

### Step 10: IMPLEMENT
- [x] Add `isProtectedLedgerPath(filePath)` to `src/hooks/PreToolUse.Edit.js` with the
      same relativize + traversal-reject logic as `isWhitelisted`.
- [x] Insert the Step-0 deny branch at the top of `enforce()` (before the whitelist
      allow), calling the existing `block(...)`.
- [x] Add `isProtectedLedgerPath` to `module.exports`.

### Step 11: REVIEW
- [x] Confirm the deny runs BEFORE the whitelist so `.ctoc/approvals/` cannot be
      allowed by the `/^\.ctoc\//` pattern; confirm no other `.ctoc/` path regresses.

### Step 12: OPTIMIZE
- [x] Predicate is a single normalize + two string checks; no regex backtracking, no
      extra fs calls.

### Step 13: SECURE
- [x] Traversal (`.ctoc/approvals/../x`) is rejected by the shared normalization.
- [x] Deny reason on stderr names the policy, leaks no absolute host path beyond the
      existing `block()` output.
- [x] Fail-open contract preserved: an internal error still exits 0 (unchanged).

### Step 14: VERIFY
- [x] `node --test tests/ctoc-audit-w02-s2-ledger-write-deny.test.js` → `# fail 0`.
- [x] `node --test tests/*.test.js` green — existing whitelist/coverage tests (e.g.
      `enforcement-hook.test.js`, `e2e-enforcement-and-gates.test.js`) still pass.

### Step 15: DOCUMENT
- [x] JSDoc on `isProtectedLedgerPath`; a comment at the `enforce()` Step-0 branch
      explaining the precedence over the `.ctoc/` whitelist and the C4 rationale.

### Step 16: FINAL-REVIEW
- [x] Verify against C4 "ledger write-denied": every editing tool routed through
