---
approved_by: human
approved_at: 2026-07-13T20:53:24.518Z
...
iron_loop: true
approved_by: human
approved_at: 2026-07-13T19:14:43.224Z
...
### Step 8: TEST
- [x] Write `tests/ctoc-audit-w05-verify-evidence.test.js` FIRST (TDD RED),
...
- [x] Confirm the suite is RED: the three functions do not exist yet, so the
      import/behavior assertions fail.

### Step 9: PREPARE
- [x] Confirm `js-yaml` is not needed here (JSON artifact); confirm `safeFs`
      and `path` are already required in `step-13-verify.js`.
- [x] Confirm `.ctoc/state/` is writable / created with `recursive: true`.

### Step 10: IMPLEMENT
- [x] `src/lib/step-13-verify.js`: add `verifyEvidencePath(projectPath, planSlug)`.
- [x] `src/lib/step-13-verify.js`: add `persistVerifyResult(projectPath, planSlug)`
      that calls `runVerify` and writes the timestamped artifact.
- [x] `src/lib/step-13-verify.js`: add `readVerifyEvidence(projectPath, planSlug)`
      returning the parsed artifact or `null` (absent/corrupt).
- [x] `src/lib/step-13-verify.js`: extend `module.exports` with the three
      functions.

### Step 11: REVIEW
- [x] Self-review: `runVerify` is now genuinely called by `persistVerifyResult`
      (zero-callers defect closed); no cycle introduced; functions are single-
      responsibility; naming matches existing camelCase.

### Step 12: OPTIMIZE
- [x] Ensure the artifact is written once per persist call; no redundant reads;
      `readVerifyEvidence` does a single `existsSync`+`readFileSync`.

### Step 13: SECURE
- [x] Verify the Security Review checklist items; add the bare-basename slug
      assertion; confirm no new shell surface.

### Step 14: VERIFY
- [x] Run `node --test tests/ctoc-audit-w05-verify-evidence.test.js` → all green.
- [x] Run the full suite `node --test tests/*.test.js` → `# fail 0`, `0 skipped`.
- [x] Confirm coverage ≥80% on the new functions.

### Step 15: DOCUMENT
- [x] JSDoc on all three new functions (params, return, throws, artifact shape).

### Step 16: FINAL-REVIEW
- [x] Confirm: real caller for `runVerify` exists; artifact round-trips;
