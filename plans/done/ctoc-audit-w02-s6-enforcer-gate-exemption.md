---
approved_by: human
approved_at: 2026-07-13T20:53:24.273Z
...
iron_loop: true
approved_by: human
approved_at: 2026-07-13T19:14:42.938Z
...
- [x] Add focused tests to `tests/iron-loop-enforcer.test.js`, real temp dir + real
      plan files (zero doubles), driving `checkGateDestinationsApproved` via
      `checkAllInvariants({ scopes: ['iron-loop'] })`:
  - [x] (a) unmarked plan in `implementation/` WITH `parent_plan:` → NOT flagged
  - [x] (b) unmarked plan in `implementation/` with NO `parent_plan` → flagged
  - [x] (c) unmarked plan in `todo/` (and `done/`) WITH `parent_plan:` → flagged
  - [x] (d) plan whose only `approved_by: human` is in the prose body → flagged
- [x] Confirm RED against current code.
- [x] Do NOT modify or weaken the existing "live repo passes" tests.

## Step 9: PREPARE

- [x] No new dependencies. Node built-in test runner only.

## Step 10: IMPLEMENT

- [x] `src/lib/iron-loop-enforcer.js` — rewrite `checkGateDestinationsApproved`:
  - [x] Extract the leading frontmatter region (handle the Gate-1 prepended
        marker-block form: two consecutive `---` blocks).
  - [x] Exempt `stage === 'implementation'` AND `parent_plan:` present in frontmatter.
  - [x] Exempt `type: vision` plans in any gate destination (decomposed archives).
  - [x] Detect `approved_by: human` / `approved_by_human: true` in the frontmatter
        region ONLY, not a raw substring.
  - [x] Keep flagging parentless unmarked `implementation/` plans and any unmarked
        `todo/` or `done/` plan.

## Step 11: REVIEW

- [x] Self-review: exemptions are minimal, no gate hole introduced.

## Step 12: OPTIMIZE

- [x] Single read per plan; regex compiled inline. No redundant IO.

## Step 13: SECURE

- [x] No path traversal (paths come from `listPlans` over the repo's own `plans/`).
      No untrusted input. Frontmatter parse is anchored, ReDoS-safe (lazy, bounded).

## Step 14: VERIFY

- [x] `node --test tests/*.test.js` → `# fail 0`, `# skipped 0`.
- [x] Live-repo fast + thorough self-check tests pass (51 slices exempt, marked
      parents still pass).
- [x] New tests pass.

## Step 15: DOCUMENT

- [x] Inline comment on the exemption explaining H7's second home + advisory nature.

## Step 16: FINAL-REVIEW

- [x] All steps complete; ready for review.
