---
approved_by: human
approved_at: 2026-07-13T20:53:24.370Z
...
- [x] **No `_shared` fragment is dispatchable**: assert `dispatchableSet(root)` contains
  none of the 4 `agents/_shared/*.md`. RED now.
- [x] **No real agent is dropped**: assert the set INCLUDES `agents/coordinator/cto-chief.md`
  and all 5 scouts (`agents/scouts/{dep,lint,secret,syntax,test}-scout.md`) plus a sample
  of others — guards against over-exclusion.
- [x] **Exclusion generalizes**: assert a hypothetical `agents/_shared/zzz-new.md` path is
  not covered by any whitelisted entry (directory-scoped, not a hardcoded 4-name list).
- [x] **Drift guard**: assert every immediate child dir of `agents/` EXCEPT `_shared`
  appears in `plugin.json.agents` (a new category cannot silently miss the manifest, and
  `_shared` must never be listed).
- [x] Confirm RED: `node --test tests/agent-shared-not-dispatchable.test.js`.

### Step 9: PREPARE
- [x] Verify against the Claude Code plugin-manifest schema whether `plugin.json.agents`
...
- [x] If it merges (whitelist cannot exclude `_shared`): stop and `markNeedsInput` with the
  relocation fallback and its broken-`../_shared/`-reference cost — do not relocate silently.
- [x] Enumerate the current immediate child dirs of `agents/` to build the exact whitelist.

### Step 10: IMPLEMENT
- [x] `.claude-plugin/plugin.json`: add `"agents"` = the 25 real category dirs (all
  `agents/*` except `_shared`), preserving all existing keys.
- [x] `tests/agent-shared-not-dispatchable.test.js`: finalize `dispatchableSet` to expand
  the manifest whitelist (already written in Step 8; ensure it reads the now-present field).

### Step 11: REVIEW
- [x] Self-review: `_shared` absent from `agents`; all 25 real categories present; JSON is
  well-formed; no existing manifest key altered.

### Step 12: OPTIMIZE
- [x] Confirm the whitelist is derived by rule (all `agents/*` minus `_shared`), the test's
  drift-guard keeps it complete, and no `_shared` path leaked in.

### Step 13: SECURE
- [x] `plugin.json` paths are repo-relative directory names only — no traversal, no user
  input. Confirm no other plugin component (commands/hooks) is disturbed.

### Step 14: VERIFY
- [x] Run the new test green: `node --test tests/agent-shared-not-dispatchable.test.js`.
- [x] Run the full suite: `node --test tests/*.test.js` — expect `# fail 0`.

### Step 15: DOCUMENT
- [x] Record the reversal (manifest whitelist, not code walker; harness does the walk) and
  the override-vs-merge verification result, so the mechanism is legible to the next reader.

### Step 16: FINAL-REVIEW
- [x] Confirm the four Story-C acceptance scenarios (no `_shared` dispatchable; real agents
