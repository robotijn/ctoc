---
approved_by: human
approved_at: 2026-07-13T20:53:25.214Z
...
### Security Review
- [x] `todo-order.json` holds only plan basenames (no paths/traversal); written atomically to
      `.ctoc/state/` (a CTOC-owned dir). `readPlans` reads it fail-safe (try/catch → fallback).
- [x] No user-web input; `planPath`/`projectPath` are internal.
- [x] Deleting dead exports removes a misleading surface (map matches territory).
- [x] `readPlans` must never throw on a corrupt `todo-order.json` (fail-safe to birthtime).

## Execution Plan (Steps 8-16)

### Step 8: TEST (TDD Red)
- [x] Write `tests/queue-order.test.js` (tests 1-4) and `tests/actions-dead-exports-guard.test.js`
      (test 5). Run — reorder test + boundary test + dead-export guard fail on current `main`.

### Step 9: PREPARE
- [x] Pre-flight: touched files == `files:` (actions.js, state.js, queue-order.test.js,
...
### Step 10: IMPLEMENT
- [x] `src/lib/state.js`: add `applyTodoOrder` hook to `readPlans` (todo-only, fail-safe).
- [x] `src/lib/actions.js`: rewrite `moveUpInQueue`/`moveDownInQueue` onto `todo-order.json`
...
### Step 11: REVIEW
- [x] Swap-order and display-order read from the SAME source (cannot diverge). Non-todo stages
      and un-ordered todo dirs behave identically to today. `initBackgroundAgent` intact.

### Step 12: OPTIMIZE
- [x] Ensure `readPlans` reads `todo-order.json` at most once per call and only for the todo
      stage.

### Step 13: SECURE
- [x] Security checklist above; confirm `readPlans` never throws on corrupt order file.

### Step 14: VERIFY
- [x] `node --test tests/queue-order.test.js tests/actions-dead-exports-guard.test.js tests/cache-freshness.test.js` — `# fail 0`.
- [x] `node --test tests/*.test.js` — `# fail 0`. Coverage ≥ 80% on changed lines.

### Step 15: DOCUMENT
- [x] Comment the new ordering key on `moveUp/DownInQueue` and `readPlans`.
- [x] **Doc-consistency flag (see Decisions):** `agents/planning/vision-decomposer.md`
