---
approved_by: human
approved_at: 2026-07-13T20:53:24.152Z
...
iron_loop: true
approved_by: human
approved_at: 2026-07-13T19:14:42.862Z
...
### Step 8: TEST (TDD Red)
- [x] Write `tests/ctoc-audit-w02-s1-approval-ledger.test.js` covering cases 1–7
...
### Step 9: PREPARE
- [x] Confirm `crypto` (Node builtin) and `./safe-fs` are the only new dependencies;
      no package install. Confirm no `src/lib/approval-ledger.js` exists yet.

### Step 10: IMPLEMENT
- [x] Create `src/lib/approval-ledger.js` exporting `ledgerDir`, `ledgerPath`,
      `slugFromPlanPath`, `computeContentHash`, `writeEntry`, `readEntry`, `verify`,
      `removeEntry` per the File Specification.
- [x] `ledgerPath` validates the slug (`/^[a-z0-9][a-z0-9-]*$/`) before joining.
- [x] `verify` enforces all three conditions (entry present, `stage_to` match, hash
      match); `readEntry` is fail-soft (`null` on absent/corrupt).

### Step 11: REVIEW
- [x] Confirm all filesystem I/O routes through `safe-fs`, all paths via `path.join`,
      and no function writes outside `.ctoc/approvals/`.

### Step 12: OPTIMIZE
- [x] Single hash computation per `verify` call; no redundant reads (one `readEntry`).

### Step 13: SECURE
- [x] Slug validation blocks path traversal into/out of `.ctoc/approvals/`.
- [x] `writeEntry` only ever targets `ledgerPath(slug)`; no caller-supplied path.
- [x] `readEntry` never leaks a stack trace on a corrupt file (returns `null`).

### Step 14: VERIFY
- [x] `node --test tests/ctoc-audit-w02-s1-approval-ledger.test.js` → `# fail 0`.
- [x] Full suite `node --test tests/*.test.js` stays green (new module, no callers
      yet — additive).

### Step 15: DOCUMENT
- [x] Module-level JSDoc stating the ledger is the single source of approval truth,
      is agent-write-denied (enforced in s2), and is written only by `approvePlan`
      (s5). Per-export JSDoc with `@param`/`@returns`/`@throws`.

### Step 16: FINAL-REVIEW
- [x] Verify against C4: `verify` returns `false` for (a) no entry, (b) hash mismatch,
