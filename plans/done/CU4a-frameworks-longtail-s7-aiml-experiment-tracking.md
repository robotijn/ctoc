---
approved_by: human
approved_at: 2026-07-10T18:13:18.273Z
...
- [x] Create `tests/cu4a-aiml-experiment-tracking-guides.test.js` (zero doubles — reads the 2 REAL guides off disk via `fs.readFileSync`)
...
- [x] Web-verify the current stable release of each of mlflow · wandb (official docs / release notes / PyPI / npm / GitHub releases)
...
- [x] Confirm `.ctoc/skills.json` still indexes the mlflow · wandb triggers (H1/frontmatter intact)
- [x] Coverage ≥ 80% (content-grounding substitutes per CU2/CU3 convention); 0 skipped, 0 flaky

### Step 15: DOCUMENT
- [x] Append per-file UPGRADED verdicts to `.ctoc/audit/corpus-audit-2026-06-15.json` (slice:"CU4a-s7") so the completeness check (s31) has no silent omissions
