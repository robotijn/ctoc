---
approved_by: human
approved_at: 2026-07-13T20:53:24.984Z
...
- [x] **Scenario: Push entry point resolves and runs (H3, happy path)**
...
- [x] **Scenario: Push reports Tier 1 failure, does not push (H3, error path)**
...
- [x] **Scenario: `push.md`'s documented flags are real (H3, edge case)**
...
- [x] **Scenario: Live long-running agent is NOT orphaned (H8, happy path)**
...
- [x] **Scenario: Genuinely dead task past the threshold IS still orphaned (H8,
...
- [x] **Scenario: Concurrency cap counts the live task (H8)**
...
- [x] **Scenario: Genuine completion is accepted (H8)**
...
- [x] **Scenario: True session restart still falls back to staleness (H8, edge
  case)**
...
- [x] **Scenario: Multi-word `--summary` persists in full (M6, happy path)**
...
- [x] **Scenario: Traversal rejected in `planActions` (M11)**
...
- [x] **Scenario: Traversal rejected in `reviewActions` (M11)**
...
- [x] **Scenario: Settings navigation key dispatches (M12, happy path)**
...
- [x] **Scenario: Settings toggle actually persists (M12)**
...
- [x] **Scenario: PostToolUse hook awaits the sync before exit (happy path)**
...
- [x] **Scenario: Sync failure is logged, hook still exits 0 (edge case)**
