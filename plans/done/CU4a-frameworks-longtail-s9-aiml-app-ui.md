---
approved_by: human
approved_at: 2026-07-10T18:13:18.324Z
...
iron_loop: true
approved_by: human
approved_at: 2026-07-10T17:01:38.883Z
...
---
iron_loop: true
title: "AI/ML data-app UIs (gradio · streamlit)"
type: implementation
...
risk_level: MEDIUM
files:
  - skills/frameworks/ai-ml/gradio.md
  - skills/frameworks/ai-ml/streamlit.md
  - tests/cu4a-aiml-app-ui-guides.test.js
---

# CU4a s9 — AI/ML data-app UIs (gradio · streamlit)

> Slice 9 of the CU4a decomposition. De-stub the 2 thin **ai-ml** framework
> guides (gradio · streamlit) from the 5-section template floor into substantive correction surfaces, in
...
> shared research spine: ML data-app UIs: rerun/session-state execution model, file-upload + path-traversal/SSRF exposure, and public-share security. Adds one content-contract test that reads the REAL guide
...
ML data-app UIs: rerun/session-state execution model, file-upload + path-traversal/SSRF exposure, and public-share security. They are disjoint by file from every other slice, so `depends_on: none`.

### Dependency Graph

```
skills/frameworks/ai-ml/gradio.md  (MODIFY: extend 5→>5)  <--tested-by-- tests/cu4a-aiml-app-ui-guides.test.js
skills/frameworks/ai-ml/streamlit.md  (MODIFY: extend 5→>5)  <--tested-by-- tests/cu4a-aiml-app-ui-guides.test.js
```
...
#### File: `skills/frameworks/ai-ml/gradio.md`
**Action:** MODIFY (extend from 5 sections to >5; no-churn on the existing 5)
**Purpose:** Trigger-loaded correction surface for gradio edits. Add these `## ` sections below the existing five (each names ≥1 concrete identifier + a dated http source ≥ 2025-01-01 for every version/security claim):
- **Component footguns** — event-listener wiring, `State` per-session, streaming/generators, queue + concurrency (`concurrency_limit`), `gr.File` handling
- **Execution** — blocking fns freeze the queue
- **Security** — `share=True` exposes a public tunnel; documented path-traversal/file-access advisory class (CWE-22) — validate `allowed_paths`; do NOT ship debug share
- **Version** — Gradio current release (v5), dated
- **References** — dated source list (each URL retrieved ≥ 2025-01-01).

#### File: `skills/frameworks/ai-ml/streamlit.md`
**Action:** MODIFY (extend from 5 sections to >5; no-churn on the existing 5)
**Purpose:** Trigger-loaded correction surface for streamlit edits. Add these `## ` sections below the existing five (each names ≥1 concrete identifier + a dated http source ≥ 2025-01-01 for every version/security claim):
- **Rerun footguns** — top-to-bottom rerun on every widget, `st.session_state` for persistence, `@st.cache_data` vs `@st.cache_resource` (mutation/stale), `st.form` to batch
- **Execution** — long tasks block the script run
- **Security** — `unsafe_allow_html=True` is XSS (CWE-79); `st.file_uploader` path handling; secrets via `st.secrets` not code (CWE-798)
- **Version** — Streamlit current release, dated
- **References** — dated source list (each URL retrieved ≥ 2025-01-01).

### Test Plan

#### Tests: `tests/cu4a-aiml-app-ui-guides.test.js`
**Action:** CREATE
...
Content-contract test cases (per file — gradio · streamlit):
...
7. **Per-framework concrete identifiers** (proves substance, not padding):
   - `gradio`: `gr.State`, `share=True`, `CWE-22`
   - `streamlit`: `session_state`, `cache_data`, `CWE-79`
...
- Every asserted CWE id (CWE-22, CWE-79) is a REAL MITRE identifier grounded in that framework's actual
...
- [x] Create `tests/cu4a-aiml-app-ui-guides.test.js` (zero doubles — reads the 2 REAL guides off disk via `fs.readFileSync`)
...
- [x] Web-verify the current stable release of each of gradio · streamlit (official docs / release notes / PyPI / npm / GitHub releases)
...
- [x] Confirm `.ctoc/skills.json` still indexes the gradio · streamlit triggers (H1/frontmatter intact)
- [x] Coverage ≥ 80% (content-grounding substitutes per CU2/CU3 convention); 0 skipped, 0 flaky

### Step 15: DOCUMENT
- [x] Append per-file UPGRADED verdicts to `.ctoc/audit/corpus-audit-2026-06-15.json` (slice:"CU4a-s9") so the completeness check (s31) has no silent omissions
