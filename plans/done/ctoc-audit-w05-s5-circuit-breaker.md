---
approved_by: human
approved_at: 2026-07-13T20:53:24.610Z
...
iron_loop: true
approved_by: human
approved_at: 2026-07-13T19:14:43.301Z
...
### Step 8: TEST
- [x] Write `tests/ctoc-audit-w05-circuit-breaker.test.js` FIRST (TDD RED): the 7
...
- [x] Confirm RED: `src/lib/circuit-breaker.js` does not exist, so every import
      fails.

### Step 9: PREPARE
- [x] Confirm `js-yaml` resolves (`require('js-yaml')`); confirm `safe-fs`
      read/write/mkdir helpers; decide the first-frontmatter-block split regex
      (`/^---\r?\n([\s\S]*?)\r?\n---/`).

### Step 10: IMPLEMENT
- [x] `src/lib/circuit-breaker.js`: create the module — `readKickbackCounts`,
...
### Step 11: REVIEW
- [x] Self-review: stateless (no module-level counter); same-step precedence;
      body/other-keys preserved; CRLF-safe; thresholds match the resolved rule;
      no cycle; no live wiring (scope).

### Step 12: OPTIMIZE
- [x] One read + one write per `recordKickback`; single escalations.json append
      only when escalating; no redundant parses.

### Step 13: SECURE
- [x] Walk the Security Review checklist; add the `__proto__`/`constructor` step
      guard; confirm serialize-before-write ordering (no half-written plan).

### Step 14: VERIFY
- [x] Run `node --test tests/ctoc-audit-w05-circuit-breaker.test.js` → green.
- [x] Run full suite `node --test tests/*.test.js` → `# fail 0`, `0 skipped`.
- [x] Coverage ≥80% on `circuit-breaker.js`.

### Step 15: DOCUMENT
- [x] JSDoc on all exports (counter shape, threshold rule, escalation shape,
      persistence semantics); header note on the deferred call site.

### Step 16: FINAL-REVIEW
- [x] Confirm: counts persist in frontmatter across simulated restart; same-step
