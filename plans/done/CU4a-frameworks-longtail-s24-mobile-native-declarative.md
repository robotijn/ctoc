---
approved_by: human
approved_at: 2026-07-10T18:13:18.614Z
...
iron_loop: true
approved_by: human
approved_at: 2026-07-10T17:01:39.055Z
...
---
iron_loop: true
title: "Native declarative UI (swiftui · jetpack-compose · compose-multiplatform)"
type: implementation
...
risk_level: MEDIUM
files:
  - skills/frameworks/mobile/swiftui.md
  - skills/frameworks/mobile/jetpack-compose.md
  - skills/frameworks/mobile/compose-multiplatform.md
  - tests/cu4a-mobile-native-declarative-guides.test.js
---

# CU4a s24 — Native declarative UI (swiftui · jetpack-compose · compose-multiplatform)

> Slice 24 of the CU4a decomposition. De-stub the 3 thin **mobile** framework
> guides (swiftui · jetpack-compose · compose-multiplatform) from the 5-section template floor into substantive correction surfaces, in
...
> shared research spine: declarative UI: recomposition/state-identity footguns, main-thread + lifecycle, and stability/skippability performance. Adds one content-contract test that reads the REAL guide
...
declarative UI: recomposition/state-identity footguns, main-thread + lifecycle, and stability/skippability performance. They are disjoint by file from every other slice, so `depends_on: none`.

### Dependency Graph

```
skills/frameworks/mobile/swiftui.md  (MODIFY: extend 5→>5)  <--tested-by-- tests/cu4a-mobile-native-declarative-guides.test.js
skills/frameworks/mobile/jetpack-compose.md  (MODIFY: extend 5→>5)  <--tested-by-- tests/cu4a-mobile-native-declarative-guides.test.js
skills/frameworks/mobile/compose-multiplatform.md  (MODIFY: extend 5→>5)  <--tested-by-- tests/cu4a-mobile-native-declarative-guides.test.js
```
...
#### File: `skills/frameworks/mobile/swiftui.md`
**Action:** MODIFY (extend from 5 sections to >5; no-churn on the existing 5)
**Purpose:** Trigger-loaded correction surface for swiftui edits. Add these `## ` sections below the existing five (each names ≥1 concrete identifier + a dated http source ≥ 2025-01-01 for every version/security claim):
- **State footguns** — `@State` vs `@StateObject` vs `@ObservedObject` (recreated views), `@Observable` (iOS 17+) vs ObservableObject, identity + `.id()`, `EnvironmentObject` crash if missing, `Task`/`.task` cancellation
- **Performance** — body recomputation, `LazyVStack`, `@MainActor`
- **Security** — Keychain not UserDefaults for secrets (CWE-312), ATS/TLS
- **Version** — Swift 6 / current iOS SDK, dated
- **References** — dated source list (each URL retrieved ≥ 2025-01-01).

#### File: `skills/frameworks/mobile/jetpack-compose.md`
**Action:** MODIFY (extend from 5 sections to >5; no-churn on the existing 5)
**Purpose:** Trigger-loaded correction surface for jetpack-compose edits. Add these `## ` sections below the existing five (each names ≥1 concrete identifier + a dated http source ≥ 2025-01-01 for every version/security claim):
- **Recomposition footguns** — unstable params → over-recomposition, `remember`/`rememberSaveable`, `derivedStateOf`, `key()`, `LaunchedEffect`/`rememberCoroutineScope` keys, state hoisting
- **Performance** — stability/skippability, `Modifier` order, lazy lists keys
- **Security** — EncryptedSharedPreferences/Keystore not plaintext (CWE-312)
- **Version** — Compose current BOM + Kotlin 2.0/K2, dated
- **References** — dated source list (each URL retrieved ≥ 2025-01-01).

#### File: `skills/frameworks/mobile/compose-multiplatform.md`
**Action:** MODIFY (extend from 5 sections to >5; no-churn on the existing 5)
**Purpose:** Trigger-loaded correction surface for compose-multiplatform edits. Add these `## ` sections below the existing five (each names ≥1 concrete identifier + a dated http source ≥ 2025-01-01 for every version/security claim):
- **Cross-platform footguns** — expect/actual for platform APIs, resource handling per target, Compose Compiler + Kotlin version lockstep, iOS interop (UIKitView), threading model differences
- **Performance** — same recomposition rules
- **Security** — platform secure storage per target (CWE-312)
- **Version** — Compose Multiplatform current release, dated
- **References** — dated source list (each URL retrieved ≥ 2025-01-01).

### Test Plan

#### Tests: `tests/cu4a-mobile-native-declarative-guides.test.js`
**Action:** CREATE
...
Content-contract test cases (per file — swiftui · jetpack-compose · compose-multiplatform):
...
- Every asserted CWE id (CWE-312) is a REAL MITRE identifier grounded in that framework's actual
...
- [x] Create `tests/cu4a-mobile-native-declarative-guides.test.js` (zero doubles — reads the 3 REAL guides off disk via `fs.readFileSync`)
...
- [x] Web-verify the current stable release of each of swiftui · jetpack-compose · compose-multiplatform (official docs / release notes / PyPI / npm / GitHub releases)
...
- [x] Confirm `.ctoc/skills.json` still indexes the swiftui · jetpack-compose · compose-multiplatform triggers (H1/frontmatter intact)
- [x] Coverage ≥ 80% (content-grounding substitutes per CU2/CU3 convention); 0 skipped, 0 flaky

### Step 15: DOCUMENT
- [x] Append per-file UPGRADED verdicts to `.ctoc/audit/corpus-audit-2026-06-15.json` (slice:"CU4a-s24") so the completeness check (s31) has no silent omissions
