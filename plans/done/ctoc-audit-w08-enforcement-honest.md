---
approved_by: human
approved_at: 2026-07-13T20:53:24.867Z
...
- [x] Given a transcript whose most recent escape-phrase occurrence is inside a
  non-`user` entry, the matcher returns no match.
- [x] Given a transcript whose most recent escape-phrase occurrence is inside a
  `user` entry, the matcher still returns that phrase (no regression).
- [x] The block message contains none of the seven canonical escape phrases.
- [x] Given a `cwd` several directories below a project root with `.ctoc/` and a
  CTOC-marked `CLAUDE.md`, the detector resolves the same `{ isCtoc, isCtocRepo
  }` result as when run from the root.
- [x] Given a project whose `package.json` declares `"name": "ctoc"`, SessionStart
  never modifies that project's `CLAUDE.md`, regardless of where the running
  hook file itself is physically located.
- [x] Given a project whose `package.json` name is not `ctoc`, SessionStart's
  injection behavior is unchanged from today (regression guard).
- [x] The injected banner text contains neither "cryptographically enforced" nor
...
- [x] **Scenario: CTOC's own block message cannot self-unlock the next edit**
...
- [x] **Scenario: A Read of CLAUDE.md cannot unlock enforcement**
...
- [x] **Scenario: A genuinely user-typed escape phrase still unlocks**
...
- [x] **Scenario: The block message no longer seeds its own unlock**
...
- [x] **Scenario: Enforcement stays active from a nested subdirectory**
...
- [x] **Scenario: Root-level detection is unchanged**
...
- [x] **Scenario: SessionStart never edits CTOC's own CLAUDE.md, from any
  install location**
...
- [x] **Scenario: SessionStart still injects into a real consumer project**
...
- [x] **Scenario: The injected session banner describes enforcement honestly**
