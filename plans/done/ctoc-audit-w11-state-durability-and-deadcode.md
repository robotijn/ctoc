---
approved_by: human
approved_at: 2026-07-13T20:53:25.238Z
...
- [x] **Scenario: Atomic audit append under concurrency (M1/M14/M15 — enforcement-log.js)**
...
- [x] **Scenario: The same durability holds for the sibling logs (M1/M14/M15 — transition-log.js
...
- [x] **Scenario: Corrupt log file is quarantined, not reset (M1)**
...
- [x] **Scenario: Exclusive agent lock under a race (M2)**
...
- [x] **Scenario: Stale lock from a dead process is still reclaimable (M2 edge case)**
...
- [x] **Scenario: setSetting preserves non-schema config blocks (M16)**
...
- [x] **Scenario: setSetting on a fresh project creates no phantom keys (M16 edge case)**
...
- [x] **Scenario: Queue reorder actually changes order (H10)**
...
- [x] **Scenario: Reorder at a queue boundary is a real no-op, not a false success (H10 edge
  case)**
...
- [x] **Scenario: Dead tab modules are deleted, and so are their tests (B1)**
...
- [x] **Scenario: Dead agent-init wrapper exports are deleted, and so are their tests (B2 —
...
- [x] **Scenario: No one-keystroke gate crossing survives in the functional-plan legacy tab
  (L7)**
...
- [x] **Scenario: No one-keystroke gate crossing survives in the review legacy tab (L8 — Gate
  3)**
...
- [x] **Scenario: The installed post-commit hook points at a real file (L9)**
