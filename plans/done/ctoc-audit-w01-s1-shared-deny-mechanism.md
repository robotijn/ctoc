---
approved_by: human
approved_at: 2026-07-13T20:53:24.051Z
...
iron_loop: true
approved_by: human
approved_at: 2026-07-13T19:14:42.745Z
...
### Security Review
- [x] No new path handling; no user path is resolved by the emitter. N/A for traversal.
- [x] `emitDeny` writes only a fixed-shape JSON built from a first-party reason string
      (no untrusted interpolation into a shell; `permissionDecisionReason` is data).
- [x] No secrets; the reason strings contain only the target path already in the payload.
- [x] Fail-open posture preserved: a require failure of the emitter crashes the hook
...
- [x] No prototype pollution: `denyDecision` returns a fresh object literal.
