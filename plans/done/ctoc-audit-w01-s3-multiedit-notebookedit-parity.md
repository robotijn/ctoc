---
approved_by: human
approved_at: 2026-07-13T20:53:24.103Z
...
iron_loop: true
approved_by: human
approved_at: 2026-07-13T19:14:42.819Z
...
### Security Review
- [x] No new path handling; `getTargetFile()` (unchanged) already normalizes/validates
      the target — no new traversal surface.
- [x] Fail-open preserved: a delegate-load failure exits 0 (tool proceeds) with a stderr
      note — matches Write's proven pattern; no new fail-closed path.
- [x] Single stdin read per process (single-consumer pipe) — no double-drain.
- [x] No secrets; no shell-out; no dynamic `require(variable)` (literal require only,
      keeps `security/detect-non-literal-require` clean).
- [x] `main()` is `require.main`-guarded so importing the module in a test never
