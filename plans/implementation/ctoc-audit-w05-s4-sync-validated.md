...
### Step 8: TEST
- [ ] Write `tests/ctoc-audit-w05-sync-validated.test.js` FIRST (TDD RED): the 4
      behavior cases with a real temp project + real settings, no doubles.
- [ ] Confirm RED: today the invalid plan is renamed into `review` and the
      function returns `{ moved:true }`.

### Step 9: PREPARE
- [ ] Confirm how `workflow.autoMoveToReview` is read (`getSetting`) so the test
      can enable it in a real temp `.ctoc/settings`.
- [ ] Confirm `plan-validator.js` has no require on `sync.js` (no cycle).

### Step 10: IMPLEMENT
- [ ] `src/lib/sync.js`: add `require` of `validateForReview` from
      `./plan-validator`.
- [ ] `src/lib/sync.js`: insert the try/catch validation gate before
      `renameSync`; return `{ moved:false, reason }` on invalid/throw; proceed to
      rename only when valid.

### Step 11: REVIEW
- [ ] Self-review: disabled short-circuit intact; invalid never renames; valid
      still renames; no cycle; result shape consistent with `s3`.

### Step 12: OPTIMIZE
- [ ] Single `validateForReview` call; no redundant `existsSync`/reads beyond
      what already exists.
...
### Step 14: VERIFY
- [ ] Run `node --test tests/ctoc-audit-w05-sync-validated.test.js` → green.
- [ ] Run full suite `node --test tests/*.test.js` → `# fail 0`, `0 skipped`.
- [ ] Coverage ≥80% on `moveToReviewAfterPush`.

### Step 15: DOCUMENT
- [ ] Update the JSDoc/comment on `moveToReviewAfterPush` to note the validation
      gate and the `{ moved:false, reason }` failure contract.

### Step 16: FINAL-REVIEW
- [ ] Confirm the post-push sync path is validated identically to the manual
