...
- [ ] **Byte-0 structural**: for every non-`_shared` `agents/**/*.md`, assert the first
  four bytes equal the literal `---\n` (byte-for-byte, `Buffer.slice(0,4)`), not a regex
  substring. RED now: 19 files fail (they start with `# `).
- [ ] **cto-chief loads its declared read-only-ish contract (runtime proxy)**: `parseByte0FM`
...
- [ ] **Each of the 5 scouts loads `model: haiku` (runtime proxy)**: for
  `dep/lint/secret/syntax/test`-scout, `parseByte0FM` returns frontmatter matching
  `/^model:\s*haiku$/m`. RED now for the same reason.
- [ ] Confirm the suite is RED for exactly these assertions: `node --test tests/agent-contract-load.test.js`.

### Step 9: PREPARE
- [ ] Confirm no other in-flight edit is touching any of the 19 files (parent
  Dependency-Risk: serialize, never concurrent-edit).
- [ ] Re-run the heading-first scan to confirm the set is still exactly these 19 (guards
...
- [ ] `agents/scouts/syntax-scout.md`
- [ ] `agents/scouts/test-scout.md`
- [ ] Per-file guard: the heading text is preserved verbatim after the closing `---`;
  a heading containing a colon/quote is never swallowed into the YAML (it moves as body,
  not frontmatter).

### Step 11: REVIEW
- [ ] Self-review each moved file: `---` at byte 0, frontmatter content byte-identical to
  before (diff shows only relocation), heading intact directly after the closing `---`,
  body unchanged.

### Step 12: OPTIMIZE
- [ ] No abstraction added; the transform is a pure relocation. Confirm no incidental
  reflow/whitespace churn beyond the single moved block.

### Step 13: SECURE
- [ ] No new inputs, no path handling, no shell-out introduced. Confirm the edit touches
  only the 19 declared files and the one test file (PreToolUse coverage stays green).

### Step 14: VERIFY
- [ ] Run the new test green: `node --test tests/agent-contract-load.test.js`.
- [ ] Run the full suite: `node --test tests/*.test.js` — expect `# fail 0`. In
...
### Step 15: DOCUMENT
- [ ] Note in the plan's completion record that all 19 contracts now load at byte 0 and
  that the cto-chief `Bash`/`Task` grant was surfaced (see parent index), not modified.

### Step 16: FINAL-REVIEW
- [ ] Confirm the four Story-A acceptance scenarios pass, the 3 tests are green, the full
