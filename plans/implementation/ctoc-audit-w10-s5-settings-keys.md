...
### Security Review
- [ ] **No new input surface:** delegation forwards the same readline `key` object
      `menu.js` already produces; `system.js` adds no parsing of untrusted data.
- [ ] **Settings write path unchanged:** persistence goes through the existing
...
- [ ] **No gate impact:** Settings toggles tune CTOC behavior; per the settings schema no
...
- [ ] **Null-safety:** `handleKey` guards `app && app.toolMode` before delegating, so a
