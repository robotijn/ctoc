...
### Security Review
- [ ] The lock is the mechanism that enforces sequential-plan execution — a security/
      correctness control. `wx` closes the double-acquire hole.
- [ ] Owner-token CAS on release prevents a foreign/stale caller from releasing another
      agent's lock.
- [ ] `isPidAlive` on Windows shells out to `tasklist` with an integer PID (no string
      interpolation of untrusted data) — unchanged, already safe.
- [ ] Fail loud on non-EEXIST fs errors (no silent acquire on a broken filesystem).

## Execution Plan (Steps 8-16)

### Step 8: TEST (TDD Red)
- [ ] Add tests 1-5 to `tests/agent-lock.test.js`. Run — test 1 (simulated race) and test 3
      (token not overwritten) fail on current `main`; test 4 (stale recovery) passes now and
      must stay passing.

### Step 9: PREPARE
- [ ] Pre-flight: touched files == `files:`. Confirm `safe-fs.writeFileSync` forwards the
      `{flag:'wx'}` options object (it does — `options` param passthrough).

### Step 10: IMPLEMENT
- [ ] `src/lib/agent-lock.js`: rewrite `acquireLock` to wx-create-then-handle-EEXIST with
      stale retry; add optional owner-token CAS to `releaseLock`/`updateLockPlan`. No stubs.

### Step 11: REVIEW
- [ ] Confirm existing callers of `releaseLock`/`updateLockPlan` still work (optional arg is
      back-compat); stale recovery preserved.

### Step 12: OPTIMIZE
- [ ] N/A (single exclusive-create write).

### Step 13: SECURE
- [ ] Security checklist above.

### Step 14: VERIFY
- [ ] `node --test tests/agent-lock.test.js` — `# fail 0`.
- [ ] `node --test tests/*.test.js` — `# fail 0`. Coverage ≥ 80% on changed lines.

### Step 15: DOCUMENT
- [ ] Update the module header (exclusive-create `wx`, owner token, CAS release).
