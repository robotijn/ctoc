...
1. `src/hooks/validate-plan-steps.js` exists and its logic is correct. It is unit-tested
   (`tests/validate-plan-steps-coverage.test.js`, `tests/hooks-remaining.test.js:170-215`)
   and it runs as a standalone script: `node src/hooks/validate-plan-steps.js <plan>`.
2. **It is absent from `.claude-plugin/hooks.json`.** It is the only file under
   `src/hooks/` that is, which is exactly why `.ctoc/reachability-baseline.json:12` lists
   it as unreachable — `liveRoots` at `reachability.js:357-378` treats a hook as live if
   and only if the manifest names it.
...
`CLAUDE.md:481` already states this accurately: "which today runs only as a standalone
...
`.ctoc/false-green-baseline.json:54-56` records three defects in that same file:
...
- It carries the exact marker **`NOT WIRED`**, matching the `NOT ENFORCED` marker
  convention `00089` establishes for the same class of correction, so both are greppable
  as one set.
...
| 4 | the two documents agree | `CLAUDE.md`'s statement and `docs/IRON_LOOP.md`'s statement both reflect the same manifest boolean. Two shipped documents disagreeing about one mechanism is the defect being repaired, and it must not be repairable in only one of them |
...
| 6 | the standalone command still works | spawn `node src/hooks/validate-plan-steps.js` against a temp plan with a mislabeled step; assert a non-zero exit and that the output names the bad label. **The document claims this command works; the test proves it** rather than trusting it |
...
`tests/hooks-remaining.test.js:170-215` by design: that test proves the script's behaviour,
...
`src/hooks/validate-plan-steps.js:200-240` (the command-line entry and its exits) and
locate the three `process.exit` sites recorded in the false-green baseline. Read
`CLAUDE.md:481` and `docs/IRON_LOOP.md:296-312`. Read `src/lib/plan-validator.js`'s
...
write to `docs/` or `.claude-plugin/` under any branch — a test that edits the document it
checks is a fence that grades its own homework.

### Step 14: VERIFY
`node --test tests/step-label-hook-claim-matches-manifest.test.js`, then the full gated
`npm test`. Lint at `--max-warnings 0`. No git operations. **Report whether case 6
...
### Step 15: DOCUMENT
The document IS the change. Additionally confirm `CLAUDE.md:481` still matches after the
...
- It does **not** remove `src/hooks/validate-plan-steps.js` from
  `.ctoc/reachability-baseline.json`. It stays legitimately unreachable until it is wired,
  and `00088` already recorded that judgment explicitly at `:477`.
- It does **not** change `src/lib/plan-validator.js` or add label-text checking anywhere
  that runs.
- It does **not** touch the compliance claims in the same class. `docs/INDEPENDENCE.md:83`
...
4. **Case 4 pins both documents to the same manifest boolean.** The defect is not "one
   document is wrong"; it is "two shipped documents disagree and nothing noticed".
   Repairing only the wrong one leaves the mechanism that allowed the disagreement.
5. **Case 6 executes the documented command instead of trusting it.** The document tells a
   reader to run something. A claim about a command is checkable by running the command,
   and this repository has just been shown what happens when a shipped instruction is
