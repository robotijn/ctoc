...
### Step 8: TEST (TDD Red)
- [ ] Write `tests/ctoc-audit-w02-s5-atomic-stamp-merged-parse.test.js` with the four
...
### Step 11: REVIEW
- [ ] Confirm the forbidden state (marked-and-resident-in-source) is unreachable: the
      marker is written only at the destination.
- [ ] Confirm `approvePlan`'s return shape and the `to==='todo'` iron-loop path are
      unchanged; confirm `parseMetadata` is behavior-identical for single-block plans.

### Step 12: OPTIMIZE
- [ ] Hash computed once over the dest bytes; `extractFrontmatterRegion` runs once per
      `parseMetadata` call.

### Step 13: SECURE
- [ ] The ledger write is on the trusted `approvePlan` code path (never a tool call),
      consistent with s2's deny-list.
- [ ] Rollback restores the original content faithfully (no partial/marked residue in
      source); `removeEntry` clears the partial ledger entry.

### Step 14: VERIFY
- [ ] `node --test tests/ctoc-audit-w02-s5-atomic-stamp-merged-parse.test.js` →
      `# fail 0`.
- [ ] `node --test tests/*.test.js` green — every existing consumer of `parseMetadata`
...
### Step 15: DOCUMENT
- [ ] JSDoc the new `approvePlan` ordering + rollback and the ADR crash-safety
      argument; comment `parseMetadata` explaining the merged-region parse and the M19
      rationale.

### Step 16: FINAL-REVIEW
- [ ] Verify against M18 (every simulated crash point ends unmarked-in-source or
