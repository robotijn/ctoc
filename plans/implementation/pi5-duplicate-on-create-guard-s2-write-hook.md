...
### Step 8: TEST (TDD Red)
- [ ] Create `tests/plan-index-duplicate-hook.test.js` with all 9 cases, driving the real
      `run(payload, deps)` with injected stub `checkDuplicate` + capturing `stderr` sink.
- [ ] Run — all fail (the exported `run` / advisory behavior does not exist yet). Confirms Red.

### Step 9: PREPARE
- [ ] Confirm `src/lib/plan-index/duplicate-guard.js` exports async `checkDuplicate`
...
### Step 10: IMPLEMENT
- [ ] Rewrite `src/hooks/PreToolUse.Write.js` from the bare delegator into the additive
      ...
      `require.main === module`.
- [ ] No stubs, no TODOs (no-stub rule). Document the summary-derivation and stdin-handoff
      choices in `## Decisions Taken Under Ambiguity`.

### Step 11: REVIEW
- [ ] Self-review against Architecture Validation: confirm the advisory guard emits NO
...
### Step 12: OPTIMIZE
- [ ] Confirm exactly ONE `await checkDuplicate(...)` per plan write; stdin read once;
      the log dir mkdir is best-effort and not on the hot path for non-plan writes.

### Step 13: SECURE
- [ ] Verify the Security Review checklist holds in the final code — especially
      "denial of enforcement" (a guard fault never suppresses the delegate) and the
      fixed-path log write.

### Step 14: VERIFY
- [ ] `node --test tests/plan-index-duplicate-hook.test.js` → `# fail 0`.
- [ ] Coverage ≥ 80% on the added hook functions; 0 skipped, 0 flaky.
- [ ] Full suite `node --test tests/*.test.js` → `# fail 0` (existing PreToolUse/Edit and
      enforcement tests still green — proves the delegation was not broken).

### Step 15: DOCUMENT
- [ ] Hook header JSDoc: the advisory-guard contract (warns, never blocks), the
      `plans/**/*.md` scope, the stderr + `.ctoc/logs/plan-index.log` sink, and the
      preserved enforcement delegation.

### Step 16: FINAL-REVIEW
- [ ] Confirm the parent acceptance criteria are proven: near-duplicate → named warning +
