...
- [ ] Create `tests/cu4a-devops-containers-guides.test.js` (zero doubles — reads the 2 REAL guides off disk via `fs.readFileSync`)
...
- [ ] Web-verify the current stable release of each of docker · podman (official docs / release notes / PyPI / npm / GitHub releases)
...
- [ ] Confirm `.ctoc/skills.json` still indexes the docker · podman triggers (H1/frontmatter intact)
- [ ] Coverage ≥ 80% (content-grounding substitutes per CU2/CU3 convention); 0 skipped, 0 flaky

### Step 15: DOCUMENT
- [ ] Append per-file UPGRADED verdicts to `.ctoc/audit/corpus-audit-2026-06-15.json` (slice:"CU4a-s28") so the completeness check (s31) has no silent omissions
