...
**Purpose:** Generate concrete execution steps (8-16) for an implementation plan.
...
- **Async overnight** — defer-and-continue when ambiguous; let review and kickback catch wrong calls.
...
Steps 12 (OPTIMIZE), 13 (SECURE), and 14 (VERIFY) carry **dual semantics**:
...
- Generate Step 12/13/14 sub-items as **attestation entries** (e.g., `[ ] Refinement loop converged for OPTIMIZE dimension (0 critical findings remaining)`) rather than single-pass checks.
...
**Severity tier:** all compiler/linter warnings, deprecation notices, and CVEs (any severity) are **critical-tier** findings (see the [warnings-are-critical rule](../../skills/agent-fragments/warnings-are-critical.md)). A clean Step 14 VERIFY attestation requires zero warnings across all toolchains.
...
| 9 | PREPARE | Prepare environment, install deps | "QUALITY", "SETUP" |
| 10 | IMPLEMENT | ALL code changes (single step) | Multiple IMPLEMENT steps |
| 11 | REVIEW | Self-review checkpoint | IMPLEMENT |
| 12 | OPTIMIZE | Performance and simplification | IMPLEMENT |
| 13 | SECURE | Security vulnerability check | IMPLEMENT |
| 14 | VERIFY | Run ALL quality checks (lint, type, tests) | Manual verification |
| 15 | DOCUMENT | Update documentation | VERIFY |
| 16 | FINAL-REVIEW | Ready for human review | VERIFY, COMMIT |

### Anti-Patterns to Avoid

- Multiple IMPLEMENT steps (merge into one with sub-items)
- "Identify coverage" in TEST step (must WRITE tests)
- VERIFY after DOCUMENT (VERIFY is Step 14, DOCUMENT is Step 15)
- Manual verification in VERIFY (belongs in FINAL-REVIEW)
- Skipping REVIEW, OPTIMIZE, or SECURE (all are mandatory)
- Using QUALITY as a step label (Step 9 is PREPARE, quality checks go in Step 14 VERIFY)
- Using COMMIT as a step label (Step 16 is FINAL-REVIEW)

### IMPLEMENT is ONE Step with Sub-items

All code changes go in Step 10. Multiple files = multiple sub-items, NOT multiple steps.

```
WRONG:
Step 10: IMPLEMENT Update file A
Step 11: IMPLEMENT Update file B
...
- **Completeness**: All steps 8-16 present with actions, all requirements covered
