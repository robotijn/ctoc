...
### Step 4b: The Deploy Ship Gate (per-crossing, not a standing flag)
...
production*. Those are two decisions and CTOC keeps them apart — but the deploy
decision is **authorized per crossing, never by a standing config flag you set here.**
`src/lib/actions.js` fires the deployment pipeline on a Gate 3 approval **only** when
that specific approval carries the deploy stamp (`options.deploy === true`). Absent
the stamp — the normal review approval — CTOC records a deploy-ready notice and
deploys nothing. There is deliberately NO persisted "always deploy on approval" flag:
one would permanently disarm the deploy gate (a setting, not a gate), so
`actions.js` refuses to honor one. Deployment setup configures the *pipeline*
(enabled, dry_run, environments, approval, rollback, notifications); it does not and
cannot arm an auto-deploy toggle.

There is nothing to ask here and nothing to write: you configure the pipeline in the
other steps, and the human authorizes each actual deploy at approval time. Make this
plain to the user so they are not surprised that a Gate 3 approval alone never ships:
to deploy, they approve WITH the deploy stamp for that crossing, or they run the
deploy manually. If `dry_run` is still `true`, say so — even a stamped crossing will
simulate, not ship, until `dry_run: false`.
