...
**NOT ENFORCED — read before relying on any control below.** Every control in this
section is opt-in via the regulatory-regime profile system. The libraries and skills
named are present and tested, but with ONE exception (Independent Verification and
Validation, which `agents/coordinator/ivv-chief.md` consults through a real
`isControlEnabled` call) **nothing evaluates whether the control is active**, so none
of these extensions actually gate a step today. Each item is marked individually below;
choosing a profile RECORDS the control, it does not enforce it. Wiring is unbuilt work
the human schedules.
...
**Step 6 DESIGN** gains:
- `skills/safety/fmeda-analyzer` when `fmeda_design` is active — bottom-up failure-modes-effects-and-diagnostic analysis with single-point-fault-metric and latent-fault-metric. **NOT ENFORCED** (no evaluator; unbuilt wiring).
...
- Clock-source posture verification recorded to `.ctoc/audit/clock-source.yaml` when `precision_time_protocol` is active (MiFID II Regulatory Technical Standard 25 sub-100-microsecond requirement). **NOT ENFORCED** (no evaluator; unbuilt wiring).
- Tool-qualification record check at `.ctoc/tool-qualification/<tool>.yaml` when `tool_qualification` is active (ISO 26262-8 §11 Tool Confidence Level — TCL2 / TCL3 tools require qualification evidence). **NOT ENFORCED** (no evaluator; unbuilt wiring).

**Step 10 IMPLEMENT** dispatches:
- `src/lib/ai-provenance.js` PostToolUse stamp when `ai_provenance_stamp` is active (European Union Artificial Intelligence Act Article 50, effective 2 August 2026). **NOT ENFORCED** (no evaluator; unbuilt wiring).
- `src/lib/data-lineage.js` for each agent dispatch when `data_lineage` is active (Basel Committee on Banking Supervision Principle 3 lineage directed-acyclic-graph). **NOT ENFORCED** (no evaluator; unbuilt wiring).

**Step 13 SECURE** gains:
- `skills/security/cra-incident-clocks` when `cra_incident_clocks` is active (European Union Cyber Resilience Act Article 14 — 24-hour early warning, 72-hour notification, 14-day final report from 11 September 2026). **NOT ENFORCED** (no evaluator; unbuilt wiring).
...
- `skills/legal/clm-obligations` when `clm_obligations_tracker` is active (Contract Lifecycle Management obligations registered at `.ctoc/contracts/obligations.yaml`). **NOT ENFORCED** (no evaluator; unbuilt wiring).
- `src/lib/irac-schema.js` enforcement on every compliance-skill finding when `irac_compliance_output` is active (Issue-Rule-Application-Conclusion legal-memo structure). **NOT ENFORCED** (no evaluator; unbuilt wiring).
...
### Cross-cutting infrastructure (libraries always present; the profile-gated ones are NOT ENFORCED)

**NOT ENFORCED — read before relying on the profile-gated items below.** These
libraries and scripts exist and are tested, but the ones that name a control
(`audit_hash_chain`, `config_baseline`, `andon_cord_halt`) are gated by no evaluator,
so activating a profile RECORDS the control and does not switch the behaviour on. Each
is marked individually. The retention sweeper, legal-hold register and spoliation-safe
deletion are libraries too; they run only where a caller invokes them, which is unbuilt
wiring the human schedules.

- **Audit hash-chain** (`src/lib/audit-chain.js`) — the library content-hashes a dispatch entry with Secure Hash Algorithm 256 and links it to the previous chain head. Verify with `verifyChain(projectRoot)`. Intended when `audit_hash_chain` is active (Securities and Exchange Commission 17a-4 / FINRA Rule 4511 audit-trail alternative). **NOT ENFORCED** (no evaluator consults the control, and no dispatch path calls the chain today; unbuilt wiring).
...
- **Configuration baseline** — `node src/scripts/release.js` writes `.ctoc/baselines/<version>/manifest.yaml` with file hashes when `config_baseline` is active. **NOT ENFORCED** (no evaluator; unbuilt wiring).
...
- **Andon-cord halt** — intended to block new dispatches when quality metrics (escape rate, process capability index, flaky tests) breach the thresholds at `.ctoc/config/andon-thresholds.yaml`, when `andon_cord_halt` is active. **NOT ENFORCED** (no evaluator; unbuilt wiring).
