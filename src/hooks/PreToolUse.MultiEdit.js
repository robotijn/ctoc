...
'use strict';
/**
 * CTOC v7 PreToolUse Enforcement Hook — MultiEdit (W01-s3, finding C3).
 *
 ...
 * sibling is the entry point — so MultiEdit enforced nothing at all.
 *
 * `enforce()` reads `tool_name` from the payload, so the enforcement log
 * distinguishes MultiEdit from Edit/Write. `getTargetFile()` in the delegate
 * already resolves `tool_input.file_path`, so no Edit.js change is needed here.
 *
