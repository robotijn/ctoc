...
| `claude:create-plan {stage}` | Create new plan in stage, enter discussion. For the implementation stage, ALLOCATE the global zero-padded number FIRST (this stakes an atomic claim, so two agents never collide): `node -e "console.log(require('{{CTOC_ROOT}}/src/lib/plan-numbering').allocatePlanNumber(process.cwd()))"` and name the file `<number>-<slug>.md` (src/lib/plan-numbering.js is the single numbering source — never hand-count). A colliding plan write (same number, different plan) is REFUSED by the PreToolUse Write hook. |
| `claude:delete {ref}` | Delete plan file, return to stage list |
| **Plan-number check** (diagnostic — NOT a menu action key) | Surface any number that names two different plans: `node -e "console.log(JSON.stringify(require('{{CTOC_ROOT}}/src/lib/plan-numbering').findNumberCollisions(process.cwd())))"` — it scans every stage and lists each number held by two or more distinct plan slugs (`[]` when clean). Report the rows; a non-zero result is existing damage to resolve by renaming one plan to a free number, never a blind auto-fix. |
| **Plan-number repair** (maintenance script — NOT a menu action key) | Assign global numbers to any UNNUMBERED implementation plans and rewrite their links: `node -e "console.log(JSON.stringify(require('{{CTOC_ROOT}}/src/lib/plan-numbering').renumberImplementationPlans(process.cwd())))"` — it gives a global zero-padded prefix to each implementation plan that lacks one (skipping numbers already in use), renames the files, and rewrites every `parent_plan`/`depends_on` reference across all stages so no link dangles. Already-numbered plans are untouched; on a fully-numbered backlog it is a no-op. |
| `claude:cleanup-exec …` | Execute a confirmed stale-plan cleanup. The real signature is `executeCleanup(proposal, root, deps = {})` — a proposal OBJECT first, the project root SECOND; the slug and action are passed as `process.argv` (never string-interpolated, so a hostile slug cannot break out of the program). **Per-plan** (`claude:cleanup-exec plan <slug> <action>`): run `node -e "const c=require('{{CTOC_ROOT}}/src/lib/stale-cleanup');const s=process.argv[1],a=process.argv[2];console.log(JSON.stringify(c.executeCleanup({plan:s,proposedAction:a},process.cwd())))" <slug> <action>`. **Category batch** (`claude:cleanup-exec category <category>`): run `node -e "const s=require('{{CTOC_ROOT}}/src/lib/menu-screens');const c=require('{{CTOC_ROOT}}/src/lib/stale-cleanup');const cat=process.argv[1];const {items}=s._buildCleanupItems(process.cwd());console.log(JSON.stringify(items.filter(i=>i.category===cat).map(i=>c.executeCleanup({plan:i.plan,proposedAction:i.proposedAction},process.cwd()))))" <category>` — the executor-side loop SP4 specified, one single-plan proposal per member (never a category branch inside the library). **Delete guard:** the `delete` action is offered ONLY for an explicitly-rejected dead-on-arrival plan; for it the per-plan proposal additionally carries `explicitlyRejected:true`, and without that flag `executeCleanup` throws by construction (the two-layer refusal — the batch never carries the flag, so a batch can never delete). src/lib/stale-cleanup.js re-derives and re-validates the target set itself — corruption-safe move-aside, never a blind delete. Then show the result and return to the inbox |
