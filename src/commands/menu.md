...
| `claude:approve {ref}` | Run `approvePlan()`, show result, return to stage list. **`approvePlan` now VALIDATES the transition (R5-B).** On a clean plan it crosses and stamps `approved_by: human`. On an INVALID transition it REFUSES by default — returns `{ ok:false, refused:true, reason, failures }` and does NOT move the plan, stamp a marker, or write a ledger entry; surface the `failures` and route to `plan {ref}` to fix. The buried **"Approve anyway"** option (only shown on a failed `validate`) is the human's explicit override: call `approvePlan(path, root, { override: { reason } })` with the human's reason. An override crosses AND records `override: true` + the reason in BOTH the ledger entry and the plan marker (a forced crossing is auditable, never a silent one). **A refusal never auto-retries with an override** — an override is always the human's deliberate act. |
