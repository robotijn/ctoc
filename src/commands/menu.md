...
| `claude:start-agent` | **WORK.** Call startAgent(), then dispatch the next todo plan as a background `implement` task via the WORK dispatch recipe. **The scheduler serializes by FILES, not by kind** (vision F1 — the old kind-based one-implement-at-a-time rule is retired): implement tasks run CONCURRENTLY (up to 5) when their declared `files:` are DISJOINT, and two plans that touch the same file serialize (scheduler reason `file-conflict`). So startAgent may report `started:true` for several file-disjoint plans, or `{ started:false, queued:true, reason }` for a plan waiting on a conflict — a queued plan stays in todo until a slot frees. Do NOT run a foreground implement loop; each plan drains as background work and completions promote the next runnable plan. **At a wave boundary** — after enqueuing the wave's implement tasks — call `enqueueWaveSync(root, { blockedBy: <the wave's implement task ids>, label })` to schedule the integration barrier; when the scheduler promotes that `sync` task (it runs ALONE, after the whole wave is done), run the integrated suite + baseline reconcile + commit, then mark it done. See the Two-Plane Protocol. |
...
A selected board row can be **cancelled** (the F1 `cancel` transition): call
`cancelTask(root, <taskId>)` — a `queued` or `running` task moves to the terminal
`cancelled` status. If the call returns an `agentTaskId`, stop that live harness
agent (killing the harness-level agent is the caller's job; the registry only records
the transition). A cancel frees the task's slot, so — exactly like a completion —
promote the scheduler's next `nextRunnable` set. Cancelling never crosses a human gate.
