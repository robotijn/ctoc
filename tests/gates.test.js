 ...
 *   - Mutating actions.js:76 — the literal `human` in the approval marker
 *     (`approved_by: human`) — breaks G2/G3/G4 (their marker value no longer trims
 *     to exactly 'human').
 *   - Mutating actions.js:68 — the HUMAN_GATES source→dest map — breaks G1
 *     (a mutated entry flips humanGate / the destination) and G5 (a stage that
 *     stopped being a source no longer throws, or one that became a source no
 *     longer throws — either way an assertion breaks).
...
/**
 * Write a minimal valid plan (no pre-existing approval marker unless `frontmatter`
 * supplies one). `frontmatter` lets G4 carry `iron_loop: true` so applyIronLoop
 * short-circuits (IL-hazard mitigation).
 */
function writePlan(root, stage, slug, { frontmatter = {} } = {}) {
...
describe('Human gates — real approvePlan gate-map contract', () => {
  ...
  it('G1 the gate map is exactly {functional→implementation, implementation→todo, review→done} (pinned via approvePlan behavior)', () => {
    ...
    for (const [from, to] of cases) {
      ...
      // iron_loop:true keeps the implementation→todo case from driving refinement.
      const planPath = writePlan(root, from, 'g1-' + from, { frontmatter: { iron_loop: true } });
  ...
  it('G2 approvePlan functional→implementation writes approved_by:human (trims to exactly "human") and moves to implementation', () => {
    const root = makeSandbox();
    const planPath = writePlan(root, 'functional', 'g-func');
  ...
  it('G3 approvePlan review→done writes marker and moves to done', () => {
    ...
    // runDeploymentPipeline never runs (actions.js:126-139).
    const { newPath, humanGate } = approvePlan(planPath, root);
  ...
  it('G5 a non-gate transition receives no gate marker (todo/done are not gate SOURCES)', () => {
    // approvePlan on a plan NOT under any flowMap source stage throws
    // "Unknown plan location" and never touches the file. This is the behavioral
    // proof that todo/done carry no gate-source key in the private HUMAN_GATES map.
