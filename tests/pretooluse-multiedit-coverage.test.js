/**
 * Dark-branch coverage for src/hooks/PreToolUse.MultiEdit.js.
 *
 * The parity test (w01-multiedit-notebookedit-parity.test.js) already proves the
 * happy path: a real MultiEdit payload spawned as the hook's OWN process entry
 ...
 * user's edit because its own machinery broke. They are unreachable through the
 * real filesystem (the sibling delegate exists and exports a function), so they
 * are driven IN-PROCESS by calling the exported main() while (a) forcing the
 * delegate require to throw via a Module._load boundary patch, and (b) injecting a
 * non-function export into the require cache — both at the true module-loader
 * boundary, no core logic mocked. process.exit / process.stderr.write are stubbed
 * at the process boundary (the hook terminates via process.exit) and restored in
 * finally.
 *
 * The remaining tests drive the ALLOW-vs-BLOCK decisions the MultiEdit hook
...
const multiedit = require(MULTIEDIT_HOOK);
...
function makeProject() {
  const dir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'ctoc-me-cov-')));
  ...
  fs.writeFileSync(
    ...
    '# CTOC Project Instructions\n\nHermetic MultiEdit coverage fixture.\n',
  );
  fs.writeFileSync(
...
function makeNonCtocDir() {
  return fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'plain-me-cov-')));
}
...
/**
 * Spawn the REAL MultiEdit hook as its own process with `rawInput` on stdin.
 ...
 */
function spawnMultiEditRaw(rawInput, dir, extraEnv = {}) {
  const res = spawnSync(process.execPath, [MULTIEDIT_HOOK], {
    cwd: dir,
    input: rawInput,
    encoding: 'utf8',
...
function spawnMultiEdit(payload, dir, extraEnv = {}) {
  return spawnMultiEditRaw(JSON.stringify(payload), dir, extraEnv);
}
...
describe('PreToolUse.MultiEdit — readStdinJson dark branches (subprocess boundary)', () => {
  let dir;
  ...
  it('readStdinJson_swallows_malformed_json_so_hook_decides_instead_of_crashing', () => {
    ...
    const out = spawnMultiEditRaw('this is not json {{{', dir);
  ...
  it('readStdinJson_returns_null_on_empty_pipe_and_hook_denies_in_ctoc_project', () => {
    ...
    const out = spawnMultiEditRaw('', dir);
...
describe('PreToolUse.MultiEdit — main() fail-open safety branches (in-process, loader boundary)', () => {
  it('main_fails_open_with_exit0_when_delegate_module_load_throws', async () => {
    ...
    assert.match(result.stderr, /enforcement delegate failed to load \(failing open\)/,
      'the fail-open path must announce the delegate-load failure on stderr');
  ...
  it('main_fails_open_with_exit0_when_delegate_export_is_not_a_function', async () => {
    ...
    assert.match(result.stderr, /enforcement delegate has no enforce\(\) \(failing open\)/,
      'the fail-open path must announce the missing enforce() on stderr');
  });
});

describe('PreToolUse.MultiEdit — delegated allow/block decisions (subprocess boundary)', () => {
  let dir;
  afterEach(() => { cleanup(dir); dir = undefined; });

  it('whitelisted_infrastructure_target_is_allowed_through_multiedit', () => {
    // Non-obvious ALLOW: an uncovered target under src/ is denied (parity test),
    ...
    // deny these, flipping denied to true.
    dir = makeProject();

    const versionOut = spawnMultiEdit({
      tool_name: 'MultiEdit',
      tool_input: { file_path: path.join(dir, 'VERSION'), edits: [{ old_string: '1', new_string: '2' }] },
    }, dir);
    const planOut = spawnMultiEdit({
      tool_name: 'MultiEdit',
      tool_input: { file_path: path.join(dir, 'plans', 'todo', 'note.md'), edits: [{ old_string: 'a', new_string: 'b' }] },
    }, dir);
  ...
  it('approval_ledger_write_is_blocked_even_though_dotctoc_is_whitelisted', () => {
    ...
    const logsOut = spawnMultiEdit({
      tool_name: 'MultiEdit',
      tool_input: { file_path: path.join(dir, '.ctoc', 'logs', 'x.json'), edits: [{ old_string: 'a', new_string: 'b' }] },
    }, dir);
    const ledgerOut = spawnMultiEdit({
      tool_name: 'MultiEdit',
      tool_input: { file_path: path.join(dir, '.ctoc', 'approvals', 'forged.yaml'), edits: [{ old_string: 'a', new_string: 'b' }] },
    }, dir);
  ...
  it('user_typed_escape_phrase_in_transcript_allows_an_otherwise_uncovered_edit', () => {
    ...
    // "quick fix"; the edit must be allowed. Mutant killed: dropping the escape
    // check (or reading it from non-user text) leaves this denied.
    dir = makeProject();
    const transcript = path.join(dir, 'transcript.jsonl');
    fs.writeFileSync(transcript,
      JSON.stringify({ type: 'user', message: { role: 'user', content: 'please just do a quick fix here' } }) + '\n');
    const targetAbs = path.join(dir, 'src', 'lib', 'uncovered.js');

    const out = spawnMultiEdit({
      tool_name: 'MultiEdit',
    ...
    assert.equal(out.denied, false,
      `a user-typed escape phrase must allow the uncovered edit; stdout=${out.stdout} stderr=${out.stderr}`);
  ...
  it('escape_word_boundary_is_respected_so_embedded_lookalike_still_blocks', () => {
    ...
    // edit must stay DENIED. Mutant killed: loosening the \b matcher to a substring
    // match would wrongly allow this, flipping denied to false.
    dir = makeProject();
    const transcript = path.join(dir, 'transcript.jsonl');
    fs.writeFileSync(transcript,
  ...
  it('non_ctoc_project_silently_passes_an_uncovered_edit', () => {
    // Non-obvious ALLOW: outside a CTOC project the hook is out of scope and must
    // silently pass — even an uncovered src/ target. Mutant killed: removing the
    // `!detect.isCtoc` early-allow would block edits in every non-CTOC repo.
    dir = makeNonCtocDir();
